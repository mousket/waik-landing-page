import { NextResponse } from "next/server"
import { withAdminAuth } from "@/lib/api-handler"
import { isEffectiveAdminFacilityError, resolveEffectiveAdminFacility } from "@/lib/effective-admin-facility"
import { buildOrGetDailyBrief } from "@/lib/admin-community-intelligence"

export const dynamic = "force-dynamic"
export const maxDuration = 45

/**
 * GET /api/admin/daily-brief?facilityId=...&organizationId=...
 * Plain JSON with `text` and `generatedAt` — 1h Redis cache per facility.
 */
export const GET = withAdminAuth(async (request, { currentUser: user }) => {
  const r = await resolveEffectiveAdminFacility(request, user)
  if (isEffectiveAdminFacilityError(r)) {
    return r.error
  }
  const { facilityId } = r
  try {
    const p = await buildOrGetDailyBrief(facilityId)
    return NextResponse.json(p, { status: 200 })
  } catch (e) {
    console.error("[daily-brief]", e)
    return NextResponse.json({ error: "Failed" }, { status: 500 })
  }
})
